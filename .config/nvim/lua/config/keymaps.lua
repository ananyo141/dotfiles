...
-- Function to close buffers to the left
local function close_buffers_to_the_left()
  local current_bufnr = vim.fn.bufnr('%')
  local buffers = vim.api.nvim_list_bufs()
  for _, bufnr in ipairs(buffers) do
    if bufnr == current_bufnr then
...
vim.keymap.set("n", "<leader>bh", close_buffers_to_the_left, { noremap = true, silent = true })
vim.keymap.set("n", "<leader>bl", close_buffers_to_the_right, { noremap = true, silent = true })
