...
local nkeymap = {
  set = function(mode, trigger, command, silent)
    vim.keymap.set(mode, trigger, command, { noremap = true, silent = silent or false })
  end,
}

-- Custom bindings
nkeymap.set("n", "<Enter>", ":w<CR>", true) -- save on enter
nkeymap.set("n", "<F7>", ":source $MYVIMRC<CR>") -- F7 sources rc file

nkeymap.set("n", "\\i", 'o#include ""<Esc>$i') -- insert user c-header
nkeymap.set("n", "\\I", "o#include <><Esc>$i") -- insert system c-header

nkeymap.set("n", "\\o", "o<Esc>k") -- insert newline one line down
nkeymap.set("n", "\\O", "O<Esc>j") -- insert newline one line up
...
nkeymap.set("n", "<C-N>", ":bnext<CR>", true)
nkeymap.set("n", "<C-P>", ":bprev<CR>", true)
